...
class GridWorld(object):
  ...
  def get_transition_mat(self):
    ...
    for si in range(N_STATES):
      ...
      for a in range(N_ACTIONS):
        probs = self.get_transition_states_and_probs(posi, a) #this gives the new state (and relative prob)
                                                              # after taking action a being in state posi
  ...
  def pos2idx(self, pos):
    ...
    returns:
      1d index
    """
    return pos[0] + int(pos[1]) * self.height
