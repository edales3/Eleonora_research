...
def direct_method(env,true_sa_visits,discount,dim_obs,reward,transition,state_only):
# this version of the direct method solves a system of linear equations (as in slide 12)
    learned_rew, learned_q = tabular_maxent_irl(env, true_sa_visits, lr=0.01, num_itrs=1000,
                                                ent_wt=1.0, state_only=state_only,
                                                discount=discount)
    learned_policy = get_policy(learned_q, ent_wt=1.0)
    ...
    eps = 10e-9
    V_irl = np.zeros(dim_obs)
    delta = 2 * eps

    while delta > eps:
        delta_max = -1
        v_prev = copy.deepcopy(V_irl)
        V_irl = reward_array_irl + discount * np.dot(transition_matrix, V_irl)
        if abs(V_irl - v_prev).any() > delta_max:
            delta_max = np.max(abs(V_irl - v_prev))
        delta = delta_max
...
if __name__ == "__main__":
    ...
    V_irl = direct_method(env,true_sa_visits,discount,dim_obs,reward,transition,state_only=True)
    V_irl_sa = direct_method(env,true_sa_visits,discount,dim_obs,reward,transition,state_only=False)
